...
public class MessageDAO extends DAO<Message> {
	...
	public void create(Message obj) {
		try {
			PreparedStatement prepare = this.connect.prepareStatement(
					"INSERT INTO utilisateur (msg_id, msg_corps, msg_date, msg_statut, msg_auteur, msg_ticket) VALUES(?, ?, ?, ?, ?, ?)");
	...
	public void update(Message obj) {
		try {
			...
			res.updateInt(4, obj.getAuteur().getId());
		} catch (SQLException e) {
			e.printStackTrace();
		}		
	}
	...
	public Message find(int id) {
		...
		try {
			...
			if (result.next())
				message = new Message(id, result.getString("msg_corps"), result.getString("msg_date"), StatutMessage.valueOf(result.getString("msg_enum")), utilisateurDAO.find(result.getInt("msg_auteur")));
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return message;
